<template>
  <div
    :class="`delete-form-${this.$store.state.mood}-${this.$store.state.language}-${this.$store.state.remove_plan_form_dash_status}`"
  >
    <div class="header">
      {{
        this.$store.state.language == "English"
          ? this.$store.state.English.remove_plan_form_dash.title
          : this.$store.state.Arabic.remove_plan_form_dash.title
      }}
    </div>

    <button class="delete" @click="DeletePlan">
      {{
        this.$store.state.language == "English"
          ? this.$store.state.English.remove_plan_form_dash.remove
          : this.$store.state.Arabic.remove_plan_form_dash.remove
      }}
    </button>

    <button
      class="cancel"
      @click="this.$store.commit(`OpenOrCloseDeletePlanForm`)"
    >
      {{
        this.$store.state.language == "English"
          ? this.$store.state.English.remove_plan_form_dash.cancel
          : this.$store.state.Arabic.remove_plan_form_dash.cancel
      }}
    </button>
  </div>
</template>
    ...
    async DeletePlan() {
      ...
      if (this.$store.state.user.user_type == "super") {
        data = {
          ...
          plan_id: this.$store.state.plan_id_for_delete,
        };
      ...
      await axios
        .delete(
          this.$store.state.user.user_type == "super"
            ? this.$store.state.APIs.plans.super.delete
            : this.$store.state.APIs.plans.admin.delete,
          { data, headers }
        )
        .then(() => {
