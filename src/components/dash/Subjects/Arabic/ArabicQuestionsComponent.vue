  ...
  >
      ...
      <h3>
        {{
          this.$store.state.language == "English"
            ? this.$store.state.English.questions.arabic
            : this.$store.state.Arabic.questions.arabic
        }}
      </h3>
    ...
    <div class="section">
      <QuestionInQuestionsComponent
        v-for="(question_data, index) in this.$store.state.arabic_questions"
        :key="index"
...
export default {
  name: "arabic-questions-component",
  data() {
    return {
      // page status
      status: "close",
  ...
  mounted() {
    // call to get arabic questiosn method
    ...
    // call to get arabicquestion method
  ...
  methods: {
    ...
    async GetQuestionsCount() {
      await axios
        .get(this.$store.state.APIs.subjects_questions.arabic.get_count)
        .then((response) => {
    ...
    async GetEnglishQuestions() {
      ...
      await axios
        .get(this.$store.state.APIs.subjects_questions.arabic.get_all, {
          params: {
            limit: this.limit,
            page: this.page,
          },
        ...
        .then((response) => {
          ...
          this.$store.state.arabic_questions = response.data.questions_data;
        })
        .catch((error) => {
          // to stop the loading animation
          this.$store.state.loading = "close";
    ...
    async GetMoreEnglishQuestions() {
      await axios
        ...
        .then((response) => {
          // set the math questions from response to questions array in store
          this.$store.state.arabic_questions = [
            ...this.$store.state.arabic_questions,
