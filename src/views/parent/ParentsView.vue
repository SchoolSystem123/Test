<template>
  ...
  >
    ...
    <div
      :class="
        this.$store.state.parents && this.$store.state.parents.length > 0
          ? 'cont-open'
          : 'cont-close'
      "
    >
      ...
      <h3 class="page-title">{{ this.$store.state.language == "English" 
        ...
        : this.$store.state.Arabic.parents_page.page_title}}</h3>
      <!-- page title  -->
      ...
      <div :class="`result-cont-${this.view_style}`">
        ...
        : this.$store.state.Arabic.parents_page.results_message}}</p>
    ...
    async GetParents() {
      await axios
        ...
        .then((response) => {
          ...
          this.$store.state.parents = [...this.$store.state.parents ,  ...response.data.parents_data];
        })
        .catch((error) => {
          // to stop the loading animation
          this.$store.state.loading = "close";
