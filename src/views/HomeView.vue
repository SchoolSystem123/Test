<template>
  <div :class="`home-${this.$store.state.mood}`">
    <div class="cont">
      ...
      <NavComponentVue />
        <!-- classes section  -->
        <h3 class="section-title">{{ this.$store.state.language == "English" 
            ? this.$store.state.English.home.classes.title 
            : this.$store.state.Arabic.home.classes.title 
          }}

          <!-- classes page link  -->
            <router-link to="/classes">{{ this.$store.state.language == "English"
      ...
      <div :class="`classes-${this.$store.state.classes.length > 0 ? 'open' : 'close'}`" >
          <div class="boxs-cont">
            <swiper class="parallax-slider" parallax grabCursor Navigation>
              <swiper-slide v-for="(class_data , index) in this.$store.state.classes" :key="index">
                <ClassComponentVue :class_data="class_data" @click="GoToClass(class_data._id)"/>
              </swiper-slide>
            </swiper>
          </div>
      </div>
        ...
        <h3 class="section-title">{{ this.$store.state.language == "English" 
          ...
          <!-- teachers page link  -->
          <router-link to="/teachers">{{ this.$store.state.language == "English"
      ...
      <div :class="`teachers-${this.$store.state.teachers.length > 0 ? 'open' : 'close'}`" >
          <div class="boxs-cont">
            <swiper class="parallax-slider" parallax grabCursor Navigation>
              <swiper-slide v-for="(teacher_data , index) in this.$store.state.teachers" :key="index">
                <TeacherComponentVue :teacher_data="teacher_data" @click="GetTeacher(teacher_data._id)"/>
              </swiper-slide>
            </swiper>
          </div>
      </div>
        ...
        <h3 class="section-title">{{ this.$store.state.language == "English" 
          ...
          <!-- teachers page link  -->
          <router-link to="/admins">{{ this.$store.state.language == "English"
      ...
      <div :class="`teachers-${this.$store.state.admins.length > 0 ? 'open' : 'close'}`" >
          <div class="boxs-cont">
            <swiper class="parallax-slider" parallax grabCursor Navigation>
              <swiper-slide v-for="(admin_data , index) in this.$store.state.admins" :key="index">
                <AdminComponentVue :admin_data="admin_data" @click="GetAdmin(admin_data._id)"/>
              </swiper-slide>
            </swiper>
          </div>
      </div>
...
export default {
  ...
  methods : {
    ...
    async GetClasses () {
        ...
        await axios.get(this.$store.state.APIs.classes.get_all)
        .then((response) => {
    ...
    async GetTeachers () {
        ...
        await axios.get(this.$store.state.APIs.teachers.get_all)
        .then((response) => {
    ...
    async GetAdmins () {
        // get teachers request
        await axios.get(this.$store.state.APIs.admins.get_all)
        .then((response) => {
