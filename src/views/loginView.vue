  ...
  <div :class="`login-page-${this.$store.state.mood}`">
    <div class="cont">
      ...
      <div class="login-form">
        ...
        <!-- title  -->
        <h3>{{ this.$store.state.language == "English"
          ? this.$store.state.English.login.title 
          : this.$store.state.Arabic.login.title}}</h3>
          <!-- info component  -->
          <InfoComponentVue />
          <!-- email label  -->
        <label for="email">{{ this.$store.state.language == "English"
          ? this.$store.state.English.login.label.label_one 
          : this.$store.state.Arabic.login.label.label_one }}</label>

          <!-- email input  -->
        <input type="email" id="email" v-model="this.email" :placeholder="`${this.$store.state.language == 'English'
        ? this.$store.state.English.login.placeholder.email
        : this.$store.state.Arabic.login.placeholder.email}`"/>

          <!-- password label  -->
        <label for="password">{{ this.$store.state.language == "English"
          ? this.$store.state.English.login.label.label_tow 
          : this.$store.state.Arabic.login.label.label_tow }}</label>

          <!-- password container -->
        <div class="password">
          <input :type="this.inputType" id="password" v-model="this.password" :placeholder="`${this.$store.state.language == 'English'
        ? this.$store.state.English.login.placeholder.password
        : this.$store.state.Arabic.login.placeholder.password}`"/>
          <icon icon="eye" :id="`${this.inputType}-eye`" @click="changeInoutType"/>
        </div>

        <!-- log n button  -->
        <button :class="`theme-${this.$store.state.theme}`" @click="Login">{{ this.$store.state.language == "English" 
          ? this.$store.state.English.login.button
          : this.$store.state.Arabic.login.button }}</button>
      </div>
    </div>
  </div>
</template>
    ...
    Login() {
      ...
      this.$store.state.loading = "open";
        ...
        axios.post(this.api, {
          ...
          window.localStorage.setItem("Ss-user" , JSON.stringify({
